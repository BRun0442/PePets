import React, { Component } from 'react'
import "../css/lostAnimals.css";
import Animal from "../components/lostAnimal/lostAnimal.jsx";
import "../css/lostAnimalsFiltros.css";

function LostAnimals() {
    return (
        <div className="container">
            <section className='filterContainer'>
                <h3>Filtros</h3>

                <section className="inputFilterContainer">
                    <p className='inputTitle'>Endereço, cidade ou CEP</p>
                    <input className='enderecoInput' type="text" />
                </section>

                <section className="inputFilterContainer">
                    <p className='inputTitle'>Nome ou ID</p>
                    <input className='enderecoInput' type="text" />
                </section>

                <section style={{ padding: "0.1rem 0 0 1rem" }}>
                    <h4>Situação do pet</h4>

                    <label className="switch">
                        <input type="checkbox"/>
                        <span className="slider round"></span>
                        <label className='switchLabel'>Perdido</label>
                    </label>

                    <label className="switch">
                        <input type="checkbox"/>
                        <span className="slider round"></span>
                        <label className='switchLabel'>Procurando Tutor</label>
                    </label>
                </section>
            </section>

            <section className="cards">
                <Animal state='Procurando tutor' location='Rua Nova Cruz, 65, Ca...' timeAgo='Há 7d' name='Nome Desconhecido' img='https://www.google.com/imgres?imgurl=https%3A%2F%2Fimg.olhardigital.com.br%2Fwp-content%2Fuploads%2F2023%2F04%2FVira-lata-caramelo.jpg&tbnid=4-_-nCC7FgoqwM&vet=12ahUKEwi_zqmBoOWDAxVVopUCHQ3MDSkQMygIegUIARCFAQ..i&imgrefurl=https%3A%2F%2Folhardigital.com.br%2F2024%2F01%2F09%2Fpro%2Fcoreia-do-sul-proibe-consumo-de-carne-de-cachorro%2F&docid=iKda4HFsdnbUXM&w=1920&h=1280&itg=1&q=cachorro&client=opera-gx&ved=2ahUKEwi_zqmBoOWDAxVVopUCHQ3MDSkQMygIegUIARCFAQ' />
                <Animal state='Perdido' location='Quitauna, Osasco, SP...' timeAgo='Há 3h' name='Tigrão' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/ac4ddb37-f207-461f-b317-a6e144f0e231.jpeg' />
                <Animal state='Perdido' location='Rua Baependy, Cam...' timeAgo='Há 9h' name='Shurato Xuxu' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/e3eb2c6c-63a7-4c4b-a476-0f6c616b08c6.jpeg' />

                <Animal state='Procurando tutor' location='Rua Nova Cruz, 65, Ca...' timeAgo='Há 7d' name='Nome Desconhecido' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/dd9abb2e-5363-4731-929e-8d6a7199cb89.jpeg' />
                <Animal state='Perdido' location='Quitauna, Osasco, SP...' timeAgo='Há 3h' name='Tigrão' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/ac4ddb37-f207-461f-b317-a6e144f0e231.jpeg' />
                <Animal state='Perdido' location='Rua Baependy, Cam...' timeAgo='Há 9h' name='Shurato Xuxu' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/e3eb2c6c-63a7-4c4b-a476-0f6c616b08c6.jpeg' />

                <Animal state='Procurando tutor' location='Rua Nova Cruz, 65, Ca...' timeAgo='Há 7d' name='Nome Desconhecido' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/dd9abb2e-5363-4731-929e-8d6a7199cb89.jpeg' />
                <Animal state='Perdido' location='Quitauna, Osasco, SP...' timeAgo='Há 3h' name='Tigrão' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/ac4ddb37-f207-461f-b317-a6e144f0e231.jpeg' />
                <Animal state='Perdido' location='Rua Baependy, Cam...' timeAgo='Há 9h' name='Shurato Xuxu' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/e3eb2c6c-63a7-4c4b-a476-0f6c616b08c6.jpeg' />

                <Animal state='Procurando tutor' location='Rua Nova Cruz, 65, Ca...' timeAgo='Há 7d' name='Nome Desconhecido' img='https://img.olhardigital.com.br/wp-content/uploads/2023/04/Vira-lata-caramelo.jpg' />
                <Animal state='Perdido' location='Quitauna, Osasco, SP...' timeAgo='Há 3h' name='Tigrão' img='data:image/jpeg;base64,' />
                <Animal state='Perdido' location='Rua Baependy, Cam...' timeAgo='Há 9h' name='Shurato Xuxu' img='https://img.quizur.com/f/img60cf7bf0c0ee79.45642897.jpg?lastEdited=1624210484' />
            </section>
        </div>
    );
}

export default LostAnimals;