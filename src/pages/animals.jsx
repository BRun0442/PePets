import React, { Component } from 'react'

import Header from '../components/header/header.jsx';
import AnimalCard from "../components/animalCard/animalCard.jsx";
import TextInput from '../components/filterTextInput/textInput.jsx';
import ToggleInput from '../components/filterToggleInput/toggleInput.jsx';
import RangeInput from '../components/filterRangeInput/rangeInput.jsx';

import animalsStyle from "./css/animals.module.css";
import animalsFilterStyle from "./css/animalsFilter.module.css";

const cardList = [                    
    <AnimalCard state='Procurando tutor' location='Rua Nova Cruz, 65, Ca...' timeAgo='Há 7d' name='Nome Desconhecido' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/dd9abb2e-5363-4731-929e-8d6a7199cb89.jpeg' />,
    <AnimalCard state='Perdido' location='Quitauna, Osasco, SP...' timeAgo='Há 3h' name='Tigrão' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/ac4ddb37-f207-461f-b317-a6e144f0e231.jpeg' />,
    <AnimalCard state='Perdido' location='Rua Baependy, Cam...' timeAgo='Há 9h' name='Shurato Xuxu' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/e3eb2c6c-63a7-4c4b-a476-0f6c616b08c6.jpeg' />,

    <AnimalCard state='Procurando tutor' location='Rua Nova Cruz, 65, Ca...' timeAgo='Há 7d' name='Nome Desconhecido' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/dd9abb2e-5363-4731-929e-8d6a7199cb89.jpeg' />,
    <AnimalCard state='Perdido' location='Quitauna, Osasco, SP...' timeAgo='Há 3h' name='Tigrão' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/ac4ddb37-f207-461f-b317-a6e144f0e231.jpeg' />,
    <AnimalCard state='Perdido' location='Rua Baependy, Cam...' timeAgo='Há 9h' name='Shurato Xuxu' img='https://viumeupet-dev.s3.sa-east-1.amazonaws.com/petThumbs/e3eb2c6c-63a7-4c4b-a476-0f6c616b08c6.jpeg' />,

    <AnimalCard state='Procurando tutor' location='Rua Nova Cruz, 65, Ca...' timeAgo='Há 7d' name='Nome Desconhecido' img='https://img.olhardigital.com.br/wp-content/uploads/2023/04/Vira-lata-caramelo.jpg' />,
    <AnimalCard state='Perdido' location='Quitauna, Osasco, SP...' timeAgo='Há 3h' name='Tigrão' img='data:image/jpeg;base64,' />,
    <AnimalCard state='Perdido' location='Rua Baependy, Cam...' timeAgo='Há 9h' name='Shurato Xuxu' img='https://img.quizur.com/f/img60cf7bf0c0ee79.45642897.jpg?lastEdited=1624210484' />,
]

function Animal() {
    return (
        <div>
            <Header />

            <div className={animalsStyle.container}>
                <section className={animalsFilterStyle.container}>
                    <h3>Filtros</h3>

                    <TextInput name="Endereço, cidade ou CEP" />
                    <TextInput name="Nome ou ID" />

                    <section className={animalsFilterStyle.inputContainer}>
                        <h4>Situação do pet</h4>

                        <ToggleInput name="Perdido" />
                        <ToggleInput name="Procurando Tutor" />
                    </section>

                    <section className={animalsFilterStyle.inputContainer}>
                        <h4>Espécie</h4>

                        <ToggleInput name="Cachorro" />
                        <ToggleInput name="Gato" />
                        <ToggleInput name="Pássaro" />
                    </section>

                    <RangeInput name="Distância máxima"/>
                </section>

                <section className={animalsStyle.cards}>
                    {cardList.map((card) => {
                        return card;
                    })}
                </section>
            </div>
        </div>
    );
}

export default Animal;